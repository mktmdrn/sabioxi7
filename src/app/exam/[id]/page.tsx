...
export default async function ExamPage({ params }: { params: { id: string } }) {
  ...
  const exam = await getLessonById(decodedId);
    ...
    return (
      <div className="min-h-screen bg-[#f7f7f7] flex items-center justify-center p-6 text-center font-sans">
        <div className="max-w-md bg-white border-2 border-duo-gray border-b-8 p-10 rounded-[2.5rem] space-y-8 shadow-sm">
          ...
          <div>
            <h1 className="text-2xl font-black text-duo-foreground uppercase italic tracking-tight">EXAMEN NO ENCONTRADO</h1>
            <p className="text-duo-gray-dark font-bold mt-2">El examen de certificación no está disponible actualmente.</p>
          </div>
          <div className="pt-4">
            <Link 
              href="/dashboard" 
